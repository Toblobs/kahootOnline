...
def actions(button):
    ...
    """Actions lambda for all buttons."""

    try:
        pass

    except BaseException as e:
        print(f'A button error occurred on {button} -> {e}')
...
class kButton(kWidget):

    """A button. Inherits from class <Widget>
       Makes a tkinter Button on __init__."""

    def __init__(self, win, bg, rel, text, font, action, screen):
        ...
        self.widget = tk.Button(win, bg = self.bg, text = self.text, font = self.font,
                                command = lambda:actions(self.action))
